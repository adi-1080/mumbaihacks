...
if ($booking.orchestration.enabled) {
    ...
    $booking.orchestration.details | ForEach-Object {
        if ($_.success) {
            Write-Host "   Success: $($_.action)" -ForegroundColor Green
            Write-Host "      Reason: $($_.reason)" -ForegroundColor Gray
        } else {
            Write-Host "   Failed: $($_.action)" -ForegroundColor Red
...
if ($queue.etas) {
    Write-Host "ETAs automatically calculated!" -ForegroundColor Green
}
...
if ($queue.data.patients -and $queue.data.patients.Count -gt 0) {
    ...
    Write-Host ""
    Write-Host "Completing consultation for Token #$tokenToComplete..." -ForegroundColor White
    ...
    Write-Host "No patients in queue to complete" -ForegroundColor Yellow
}
...
if ($monitoring.data.actions -and $monitoring.data.actions.Count -gt 0) {
    ...
    $monitoring.data.actions | ForEach-Object {
        Write-Host "   Action: $($_.action): $($_.reason)" -ForegroundColor White
    }
}
...
if ($currentQueue.data.patients -and $currentQueue.data.patients.Count -gt 0) {
    ...
    Write-Host "No patients in queue for location update test" -ForegroundColor Yellow
}
...
Write-Host "The backend now matches root_agent.py intelligent behavior!" -ForegroundColor Cyan
