...
Write-Host "`n$separator80" -ForegroundColor Cyan
Write-Host "        INTELLIGENT ORCHESTRATION SYSTEM - DEMONSTRATION" -ForegroundColor Cyan
Write-Host "$separator80`n" -ForegroundColor Cyan
...
# ========================================
Write-Host "`n[TEST 1] HIGH-URGENCY BOOKING WITH AUTO-ORCHESTRATION" -ForegroundColor Yellow
Write-Host $separator60 -ForegroundColor Yellow

Write-Host "`n📋 Booking CRITICAL emergency patient (urgency=9)..." -ForegroundColor White
...
if ($booking.orchestration.enabled) {
    Write-Host "`n🧠 AUTOMATIC ORCHESTRATION TRIGGERED:" -ForegroundColor Cyan
    Write-Host "   Actions executed: $($booking.orchestration.actions_executed)" -ForegroundColor White
    
    Write-Host "`n📊 Orchestration Details:" -ForegroundColor Yellow
...
# ========================================
Write-Host "`n`n[TEST 2] SMART QUEUE STATUS (Auto-includes ETAs + Intelligence)" -ForegroundColor Yellow
Write-Host $separator60 -ForegroundColor Yellow

Write-Host "`n📊 Fetching smart queue status..." -ForegroundColor White
...
if ($queue.data.patients -and $queue.data.patients.Count -gt 0) {
    Write-Host "`n📋 Current Queue:" -ForegroundColor Cyan
...
# ========================================
Write-Host "`n`n[TEST 3] PATIENT COMPLETION WITH AUTO-ORCHESTRATION" -ForegroundColor Yellow
Write-Host $separator60 -ForegroundColor Yellow
...
if ($queue.data.patients -and $queue.data.patients.Count -gt 0) {
    ...
    Write-Host "`n👨‍⚕️ Completing consultation for Token #$tokenToComplete..." -ForegroundColor White
    ...
    Write-Host "`n🔄 Verifying queue updated..." -ForegroundColor Yellow
...
# ========================================
Write-Host "`n`n[TEST 4] QUEUE MONITORING ORCHESTRATION" -ForegroundColor Yellow
Write-Host $separator60 -ForegroundColor Yellow

Write-Host "`n🔍 Executing queue health monitoring..." -ForegroundColor White
...
# ========================================
Write-Host "`n`n[TEST 5] LOCATION UPDATE WITH AUTO-ORCHESTRATION" -ForegroundColor Yellow
Write-Host $separator60 -ForegroundColor Yellow
...
if ($currentQueue.data.patients -and $currentQueue.data.patients.Count -gt 0) {
    ...
    Write-Host "`n📍 Updating location for Token #$tokenToUpdate (Priority: $($highPriorityPatient.emergency_level))..." -ForegroundColor White
...
# SUMMARY
# ========================================
Write-Host "`n`n$separator80" -ForegroundColor Green
...
Write-Host "`n📚 Documentation: backend/INTELLIGENT_ORCHESTRATION_GUIDE.md" -ForegroundColor Yellow
Write-Host "🎯 The backend now matches root_agent.py's intelligent behavior!" -ForegroundColor Cyan

Write-Host "`n$separator80`n" -ForegroundColor Green
